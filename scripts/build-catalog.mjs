...
// ---------------------------------------------------------------------------
// star name list for the grounding guard
//
// grounding.ts checks every proper name in a Granite answer against the sky
// context. Because buildSkyContext never receives stars (only the Sun, Moon,
// planets and satellites), any star name in an answer is an unsupported claim
// by definition. The guard needs the full name list as a plain import so it
// works inside Vercel's serverless functions, where public/ is served as CDN
// assets and is not present on the function's filesystem.
//
// The list is derived here from the freshly-written stars.json so it stays in
// sync automatically: running `npm run catalog` regenerates both.
// ---------------------------------------------------------------------------

async function buildStarNameList() {
  const starsJson = JSON.parse(await readFile(join(OUT, 'stars.json'), 'utf8'));
  const properIndex = starsJson.fields.indexOf('proper');
  if (properIndex === -1) throw new Error('stars.json has no "proper" field: catalogue format changed');

  const names = starsJson.stars
    .map((row) => row[properIndex])
    .filter((n) => typeof n === 'string' && n.trim())
    .map((n) => n.trim());

  // Deduplicate: HYG occasionally has two entries (components) for the same
  // named star (e.g. Rigil Kentaurus / Toliman for Alpha Centauri A and B).
  ...
  const lines = [
    ...
    ...unique.map((n) => `  ${JSON.stringify(n)},`),
    '];',
    '',
  ];

  const dest = join(ROOT, 'api', '_lib', 'star-names.ts');
  await writeFile(dest, lines.join('\n'), 'utf8');
  console.log(`  star-names.ts   ${unique.length} names -> api/_lib/star-names.ts`);
}
...
async function main() {
  ...
  // buildStarNameList reads the stars.json written by buildStars, so it must
  // run after buildStars completes.
  await buildStarNameList();
  console.log('\ndone.');
}
